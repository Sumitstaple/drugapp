...
import { insertNewSpot,checkSpot } from '../databases/Schema';

class AddTrapSpotScreen extends Component {
...
createNewContact(props){
    ...
    checkSpot(this.state.name).then((data) => {
       console.log(data);
         if(data.length > 0){
            this.setState({
              nameError:'Spot Already Exists!'
            })
          }
          else{
              const userData = {
              ...
              insertNewSpot(userData).then((success) => {
               
               this.props.navigation.navigate('Trap Spots', {
                      created: 'New Trap Spot Created',
                    }); 
              }).catch((error) => {
        ...
        <View style={styles.searchcontact}>
          <KeyboardAvoidingView enabled>
            ...
            <TouchableOpacity
              ...
              <Text style={styles.buttonTextStyle} onPress={() => this.createNewContact()}>Add Spot</Text>
              }
              {this.state.showbtn == "false" &&
              <Text style={styles.btnstyle}>Add Spot</Text>
              }
            </TouchableOpacity>
...
export default AddTrapSpotScreen;
